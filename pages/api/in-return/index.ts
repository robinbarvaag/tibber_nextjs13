...
export default async function CurrentElectricalPrice(
  ...
  if (req.query.year === "2022") {
    ...
    const septemberPrices = await fetchForMonth("09", req.query.year as string);
    const oktoberPrices = await fetchForMonth("10", req.query.year as string);
    const novemberPrices = await fetchForMonth("11", req.query.year as string);
    const desemberPrices = await fetchForMonth("12", req.query.year as string);
  ...
  if (req.query.year === "2023") {
    ...
    const februaryPrices = await fetchForMonth("02", req.query.year as string);
  ...
  fetchForMonth(req.query.month as string, req.query.year as string);
}
